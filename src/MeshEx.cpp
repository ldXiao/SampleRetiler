...
{
    this->normal = Eigen::RowVector3d( 0.0, 0.0, 0.0 );
    for( std::vector<Triangle *>::iterator it= this->triangleRing.begin(); it != this->triangleRing.end(); ++it )
        normal += (*it)->normal;
    normal.normalize();
}
...
//
bool MeshEx::Edge::intersectsPlane( const Eigen::RowVector3d &normal, const float &distance, Eigen::RowVector3d &intersectionPoint )
{
