...
CGAL::Point_3<K> Point_3(const Eigen::RowVector3d &vec);
...
///
    ...
    public:
        ...
        struct Vertex {
            ...
            int getPlaneSide(const Eigen::RowVector3d &normal,
                             const double &distance);  // returns 1 if the node lies on the left side of the given plane (nodeplanedistance < 0.0f) or 2 otherwise - returns 0 if point lies on the plane
        ...
        struct Edge {
            Edge(Vertex *_v1,
                 Vertex *_v2);                                                                            // constructor

            void registerTriangle(
                    Triangle *t);  // assigns the given element to the left or right wing of the edge (dependand on which wing is 0)
            void unregisterTriangle(
                    Triangle *t);  // sets either the left or right reference to zero when it references the given element
            bool intersectsPlane(const math::Vec3f &normal, const float &distance,
                                 math::Vec3f &intersectionPoint);  // convinience function to compute the intersection of the edge with a given plane
            bool intersectsLine(const math::Vec3f &p1, const math::Vec3f &p2,
                                math::Vec3f &intersectionPoint);  // convinience function to compute the intersection of the edge with a given line
            bool intersectsLine(const math::Vec3f &p1,
                                const math::Vec3f &p2);  // convinience function to compute the intersection of the edge with a given line without giving back the intersection point
            Vertex *getOtherVertex(
                    Vertex *v);  // convinience function to get the other node of the two nodes which are referenced by the edge
        ...
        struct Triangle {
            ...
            math::Vec3f convertFromLocalToGlobalSpace(const math::Vec2f &localSpace,
                                                      bool isVector = false); // transforms the given coordinate in local 2d space into the global 3d space in which the triangle/element lies
            math::Vec2f convertFromGlobalToLocalSpace(const math::Vec3f &globalSpace,
                                                      bool isVector = false); // inverse operation of the above
            void computeProperties(); // computes normal, area, etc...
            Vertex *
            getOtherVertex(Vertex *vertex1, Vertex *vertex2); // returns the third node which neither equals n1 nor n2
            Vertex *getOtherVertex(Edge *edge); // returns the third node which neither equals n1 nor n2
            bool
            contains(Vertex *vertex) const; // returns true if one of the triangles vertices equals the given vertex
            size_t getVertexsLocalIndex(Vertex *vertex); // returns the local index of the node within the element
            Edge *getSharedEdge(
                    Triangle *t); // returns the edge which is shared with the given triangle or 0 if they dont share an edge
            Edge *getEdge(Vertex *vertex1,
                          Vertex *vertex2); // returns the edge of the triangle which contains the 2 given vertices
        ...
        Vertex *createVertex(
                const math::Vec3f &position); ///< creates a Vertex and adds it to the node list with the given world position
        void removeVertex(Vertex *vertex); ///< removes given node from the node list
        Triangle *createTriangle(const size_t &index0, const size_t &index1,
                                 const size_t &index2); ///< creates a Triangle and adds it to the element list with the given node indices
        Triangle *createTriangle(Vertex *v0, Vertex *v1,
                                 Vertex *v2); ///< creates a Triangle and adds it to the triangle list from given vertices
        Triangle *createTriangle(Vertex *v0, Vertex *v1, Vertex *v2, Edge *e0, Edge *e1,
                                 Edge *e2); ///< creates a Triangle and adds it to the triangle list from given edges
        Triangle *createTriangle(Vertex *v0, Vertex *v1, Vertex *v2,
                                 std::vector<Edge *> &edges); ///< creates a Triangle and adds it to the triangle list -> edges are looked up within the given edges (for speedup)
        Edge *createEdge(Vertex *v1, Vertex *v2); ///< creates a new edge from 2 given m_vertices
        Edge *findEdge(Vertex *n1,
                       Vertex *n2); ///< looks for an edge between the given nodes - returns 0 if it could not be found
        Vertex *splitEdge(Edge *edge,
                          const math::Vec3f &splitPosition); ///< splits the given edge into 2 and returns the splitnode which was created to seperate the edge
        void removeEdge(Edge *edge); ///< removes edge from the list of edges
        void removeTriangle(Triangle *element, bool removeVertices = true,
                            bool removeEdges = true); ///< removes element from the list of elements
        void replaceVertexWithinTriangle(Triangle *e, Vertex *originalVertex,
                                         Vertex *replacement); ///< replaces all occurances of the original node within element with the replacementnode and changes the edges accordingly
        void reTriangulate(Triangle *t,
                           const std::vector<math::Vec3f> &pointsWithin); ///< retriangulates the given triangle taking the given points into acount (which have to be coplanar with the triangle)
        bool removeVertexAndReTriangulateNeighbourhood(
                Vertex *v); ///< removes the given vertex and retriangulate the hole which would be made
        void createTrianglesFromEdges(std::vector<Edge *> &edges,
                                      const math::Vec3f &normal); ///< this method takes a list of already created edges and creates triangles for filling triangulated areas

        bool findClosestIntersection(const math::Vec3f &position, const math::Vec3f &p1, const math::Vec3f &p2,
                                     math::Vec3f &intersection, math::Vec3f &normal, Triangle *&t);

        void retriangulateHole(std::vector<MeshEx::Edge *> &boundaryEdges,
                               std::map<MeshEx::Vertex *, math::Vec2f> &boundaryVertexProjections,
                               std::vector<std::pair<math::Vec3f, math::Vec2f> > &interiorPoints = std::vector<std::pair<math::Vec3f, math::Vec2f> >());
